...
namespace BINAPI_NAMESPACE::ws
{
    ...
    class BINAPI_EXPORT WSFuturesBinanceAPI : public WSThread
    {
    public:
        /// \brief Construct a new WSBinanceAPI object

        WSFuturesBinanceAPI(std::string endPoint);
        ~WSFuturesBinanceAPI() override = default;
...
#pragma clang diagnostic ignored "-Wdocumentation-unknown-command" // Just to get rid of the warning in these lines: /ws/btcusdt@markPrice@{}1s AND /ws/btcusdt@markPrice@{}1s
        ...
        static std::string constructEndPoint(Args &...args)
        {
            std::string ret;
            int count = 0;
        ...
        static std::string constructEndPointFromList(const std::vector<std::string> &subscriptions) noexcept;
